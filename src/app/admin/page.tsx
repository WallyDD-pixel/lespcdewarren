...
export default function AdminDashboard() {
  ...
  const recentStoreOrders = (data?.recentStoreOrders ?? []) as Array<{
    id: number;
  ...
  const chartData = useMemo(
    () => ({
      ...
      datasets: [
        {
          label: "Chiffre d'affaires boutique (€)",
    ...
    <div className="grid gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button className="btn-ghost mr-2" onClick={() => (window.location.href = "/admin/contest/manage")}>
      ...
      <div className="grid gap-4 sm:grid-cols-5">
        ...
        <div className="card p-4">
          <div className="text-sm text-white/70">Commandes (payées)</div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="card p-4">
          <div className="text-sm text-white/70">Chiffre d'affaires boutique</div>
          <div className="mt-1 text-3xl font-semibold">
            {(revenueCents / 100).toLocaleString("fr-FR", { style: "currency", currency: "EUR" })}
          </div>
        </div>
        <div className="card p-4">
          <div className="text-sm text-white/70">Note moyenne avis</div>
          <div className="mt-1 text-3xl font-semibold">{avgRating.toFixed(2)} / 5</div>
        </div>
      </div>

      <AdminDashboardChartsClient chartData={chartData} days={days} />

      <div className="mt-2">
        <div className="mb-2 text-sm uppercase tracking-wide text-white/50">Boutique</div>
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="section-contrast p-4">
            <h3 className="mb-3 text-base font-semibold">Dernières commandes</h3>
