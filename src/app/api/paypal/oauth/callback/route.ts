...
export async function GET(req: NextRequest) {
  ...
  if (!code) {
    return NextResponse.redirect(new URL(`/account?error=paypal_oauth_cancelled`, req.url));
  }
  ...
  if (!sessData.state || sessData.state !== state) {
    ...
    return NextResponse.redirect(new URL(`/account?error=paypal_oauth_state`, req.url));
  }
  ...
  try {
    ...
    if (!email || !payerId) {
      return NextResponse.redirect(new URL(`/account?error=paypal_oauth_missing`, req.url));
    }

    const target = new URL(`/account`, req.url);
    ...
    // purge et erreur
    try { delete (session as any).paypal_oauth; await (session as any).save?.(); } catch {}
    return NextResponse.redirect(new URL(`/account?error=paypal_oauth_error`, req.url));
  }
}
